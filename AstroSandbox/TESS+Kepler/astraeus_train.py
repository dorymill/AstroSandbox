...
for i in range(0,len(targets)):
    ...
    try:
        ...
        lc_clean1 = lcs.stitch(corrector_func=lambda x: x.remove_outliers(sigma=10, sigma_upper=1))
        lc_clean1 = lc_clean1.flatten()
        #lc_flat_clip = lc_clean1.remove_outliers(sigma_lower=10, sigma_upper=2)
        lc_clean = lc_clean1.normalize()
        ...
        plt.tight_layout()
        plt.savefig('Training\%s.png' %title[i])
        plt.clf()   
        #plt.show()
        ...
        gdf.to_csv('Training\%s_global.csv' %title[i])
        ldf.to_csv('Training\%s_local.csv' %title[i])
